  ...
  methods: {
    ...
    fillDatasets (input) {
      ...
      Object.values(apiData).forEach((value, key) => {
        if (input === 'max') {
          dataMock.push(value.device[this.farmUser].humidity[0].max)
        } else if (input === 'min') {
          dataMock.push(value.device[this.farmUser].humidity[0].min)
        } else {
          dataMock.push(value.device[this.farmUser].humidity[0].avg)
        }
      })
      return dataMock
    },
